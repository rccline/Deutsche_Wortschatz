...
shinyServer(function(input, output, session){
  ...
  vals_trich$Data<-readRDS("note_words.rds") ## Changed note.rds to note_words.rds
  ...
  observeEvent(input$Updated_trich,{
    saveRDS(vals_trich$Data, "note_words.rds")
 ...
 ### can download the table in csv
  output$Trich_csv<- downloadHandler(
    filename = function() {
      paste("deutsche_wortschatz_", Sys.Date(), ".csv", sep="")
    },
